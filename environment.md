...
- **不用 AgentOS 版**：赛题原文全文无 "AgentOS" 字样，通篇要求 openKylin 本体（操作系统上编译
  ...
  初期载体为 2.0-SP2-AgentOS 系列 ISO；**2026-09-07 社区推送基于 3.0 的新一轮 AgentOS 更新**
  （官网新闻 4122：可靠性主题，修复系统智能体/CUA/统一模型推理服务/运行时 30+ 缺陷，新增语音交互），
  镜像站 releases/3.0/ 目录新增 `openKylin-Desktop-V3.0-AgentOS-20260905-x86_64.iso`（6.9G，
  与我们基线同 20260905 批次），获取入口 = 官网"下载 → 生态版本 → AgentOS"——AgentOS 基座已追平 3.0。
  基线仍选标准桌面版：赛题只认 openKylin 本体，且官网发布会"体验通道"显示智能体
  组件可装在 2.0 SP2 / 3.0 Alpha1 软件商店（搜"智能体"或"kylin-agent"），即组件与基座解耦，
  3.0 正式版作评测环境成立。评分细则"与 openKylin 智能体操作系统架构良好集成"按 3.0 产品定位理解。
  AgentOS 版预装更全的智能体栈（系统智能体/CUA/统一模型推理服务/智能体运行时），其推理服务与
  运行时日志可能是 §7 摸底想要的可观测面，可选作辅助环境或第二/第三智能体的获取来源，
  不作基线；报名后在官方答疑渠道再确认一次口径（2026-09-23 已核实）。
